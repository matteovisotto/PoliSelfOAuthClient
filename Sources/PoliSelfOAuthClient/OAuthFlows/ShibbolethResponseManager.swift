//
//  ShibbolethResponseManager.swift
//  poliOrarioTest
//
...
class ShibbolethResponseManager {
    ...
    public func execute(_ completionHandler: @escaping (_ result: Bool, _ url: URL?, _ htmlContent: String?)->()) -> Void {
        let shibbolethTask = self.session.dataTask(with: self.request) { data, response, error in
            if let _ = error {
